...
func TestCreateCar(t *testing.T) {
	...
	// 设置模拟行为
	mockStub.On("CreateCompositeKey", cc.CAR, []string{string(cc.AVAILABLE), "CAR001"}).Return("CAR_AVAILABLE_CAR001", nil)
	mockStub.On("GetState", "CAR_AVAILABLE_CAR001").Return([]byte(nil), nil)
	mockStub.On("CreateCompositeKey", cc.CAR, []string{string(cc.IN_TRANSACTION), "CAR001"}).Return("CAR_IN_TRANSACTION_CAR001", nil)
	mockStub.On("GetState", "CAR_IN_TRANSACTION_CAR001").Return([]byte(nil), nil)
	mockStub.On("CreateCompositeKey", cc.CAR, []string{string(cc.SOLD), "CAR001"}).Return("CAR_SOLD_CAR001", nil)
	...
	mockClientIdentity.On("GetMSPID").Return(cc.CAR_DEALER_ORG_MSPID, nil)
	...
	contract.GetClientIdentityMSPID = func(ctx contractapi.TransactionContextInterface) (string, error) {
		return cc.CAR_DEALER_ORG_MSPID, nil
	}
...
func TestCreateTransaction(t *testing.T) {
	...
	mockStub.On("CreateCompositeKey", cc.TRANSACTION, []string{string(cc.PENDING), "TX001"}).Return("TRANSACTION_PENDING_TX001", nil)
	mockStub.On("PutState", "TRANSACTION_PENDING_TX001", mock.Anything).Return(nil)

	// 替换原始的 getClientIdentityMSPID 方法
	originalGetClientIdentityMSPID := contract.GetClientIdentityMSPID
	contract.GetClientIdentityMSPID = func(ctx contractapi.TransactionContextInterface) (string, error) {
		return cc.TRADE_ORG_MSPID, nil
	}
	defer func() { contract.GetClientIdentityMSPID = originalGetClientIdentityMSPID }()
...
func TestCompleteTransaction(t *testing.T) {
	...
	mockStub.On("CreateCompositeKey", cc.TRANSACTION, []string{string(cc.COMPLETED), "TX001"}).Return("TRANSACTION_COMPLETED_TX001", nil)
...
func TestQueryCarList(t *testing.T) {
	...
	// 设置模拟行为
	mockStub.On("GetStateByPartialCompositeKeyWithPagination", cc.CAR, []string{"AVAILABLE"}, int32(10), "").Return(mockIterator, mockMetadata, nil)
...
func TestAddCertificate(t *testing.T) {
	...
	// 设置模拟行为
	mockStub.On("CreateCompositeKey", cc.CERTIFICATE, []string{"CERT001"}).Return("CERTIFICATE_CERT001", nil)
